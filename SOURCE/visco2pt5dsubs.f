...
10	format(a1)
	...
	subroutine matrel(s,ky,nmat,Ap,Ai,jAi,AA0,AA1,AA2)
...
cOLD        parameter (nzmax = 9500000, nmax = 160000)
        parameter (nzmax = kmax, nmax = 3*(N*ncmax-(ncmax-1))*(N*ncmaz-(ncmaz-1))+1)
	integer*8 Ap(nmax),Ai(nzmax),jAi(kmax)
...
c	kyone is a placeholder for unit ky so we can keep track of the
...
c	i1,j2 terms
	...
	j=jAi(k)
	AA1(j)=AA1(j)
     &	- wt2(l)*dphi2x(il,l)*lam*ui*kyone*phi2(jl,l) * dble(dz(ncz)/2.) * fac2 
...
c	i2,j1 terms
	...
	j=jAi(k)
	AA1(j)=AA1(j)
     &	- wt2(l)*dphi2x(il,l)*mu*ui*kyone*phi2(jl,l) * dble(dz(ncz)/2.) * fac2
...
c	i2,j3 terms
	...
	j=jAi(k)
	AA1(j)=AA1(j)
     &	- wt2(l)*dphi2z(il,l)*mu*ui*kyone*phi2(jl,l) * dble(dx(ncx)/2.) * fac2 * fac
	AA1(j)=AA1(j)
     &			    - wt2(l)*phi2(il,l)*rhoval*g0*ui*kyone*phi2(jl,l) * dble(dz(ncz)*dx(ncx)/4.) * fac2 * fac
c	Above line: gravitational term
...
c	i3,j1 terms
	...
	AA0(j)=AA0(j)
     &			    + wt2(l)*phi2(il,l)*rhoval*g0*dphi2x(jl,l) * dble(dz(ncz)/2.)
     &			    + wt2(l)*phi2(il,l)*rhoval*g0*phi2(jl,l)*cott * dble(dz(ncz)*dx(ncx)/4.)
c	Above line: gravitational term
...
c	i3,j2 terms
	...
	j=jAi(k)
	AA1(j)=AA1(j)
     &	- wt2(l)*dphi2z(il,l)*lam*ui*kyone*phi2(jl,l) * dble(dx(ncx)/2.) * fac2 * fac
	AA1(j)=AA1(j)
     &			    + wt2(l)*phi2(il,l)*rhoval*g0*ui*kyone*phi2(jl,l) * dble(dz(ncz)*dx(ncx)/4.) * fac2 * fac
c	Above line: gravitational term
...
c	i1,j2 terms
	...
	j=jAi(k)
	AA1(j)=AA1(j)
     &	+ wt2(l)*phi2(il,l)*fac2*ui*kyone *mu*dphi2x(jl,l) * dble(dz(ncz)/2.)
     &	- wt2(l)*phi2(il,l)*fac2*ui*kyone *mu*phi2(jl,l)*cott * dble(dz(ncz)*dx(ncx)/4.)
...
c	i2,j1 terms
	...
	j=jAi(k)
	AA1(j)=AA1(j)
     &	+ wt2(l)*phi2(il,l)*fac2*ui*kyone *lam2mu*phi2(jl,l)*cott * dble(dz(ncz)*dx(ncx)/4.)
     &	+ wt2(l)*phi2(il,l)*fac2*ui*kyone *lam*dphi2x(jl,l) * dble(dz(ncz)/2.)
...
c	i2,j3 terms
	...
	j=jAi(k)
	AA1(j)=AA1(j)
     &	+ wt2(l)*phi2(il,l)*fac2*ui*kyone *lam2mu*phi2(jl,l) * dble(dz(ncz)*dx(ncx)/4.)
     &	+ wt2(l)*phi2(il,l)*fac2*ui*kyone *lam*phi2(jl,l) * dble(dz(ncz)*dx(ncx)/4.)
     &	+ wt2(l)*phi2(il,l)*fac2*ui*kyone *lam*dphi2z(jl,l) * dble(dx(ncx)/2.) * fac
...
c	i3,j2 terms
	...
	j=jAi(k)
	AA1(j)=AA1(j)
     &	+ wt2(l)*phi2(il,l)*fac2*ui*kyone *mu*dphi2z(jl,l) * dble(dx(ncx)/2.) * fac 
     &	- wt2(l)*phi2(il,l)*fac2*ui*kyone *mu*phi2(jl,l) * dble(dz(ncz)*dx(ncx)/4.)
     &	- wt2(l)*phi2(il,l)*fac1 *lam2mu*ui*kyone*phi2(jl,l) 
     &	* dble(dz(ncz)*dx(ncx)/4.) * fac2 * fac
     &	- wt2(l)*phi2(il,l)*lam*ui*kyone*phi2(jl,l) * dble(dz(ncz)*dx(ncx)/4.) * fac2 
c	Above 1 line: Additional terms arising from r- and theta-derivatives of unit vectors.
...
30	continue
	...
	k=k0+1
	j=jAi(k)
	AA0(j)=phi2(jl,il)*roff+dphi2x(jl,il)*(2.d0/dx(ncx))
	k=k+1
	j=jAi(k)
	AA0(j)=phi2(jl,il)*roff+dphi2x(jl,il)*(2.d0/dx(ncx))
	k=k+1
...
c
	...
	real*8 dmax(6000),dmin(6000),dip(6000)
	real*8 flat(6000),flon(6000),fbigl(6000),fstr(6000),frake(6000),fwt(6000)
...
c	Further assume that the imaginary part of s varies quadratically.
	...
	real*8 t,v
	real*8 s0
	complex*16 ui,s,f(14),ds,fn,gn,val
	real*8 sarr(7)
