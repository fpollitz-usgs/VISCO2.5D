...
c	when considering free air gravity computations.
	...
	complex*16 gravz(ncmax*N,41,12),pb(ncmax*N,2001)
	complex*16 sumg1(2000),sumg2(2000)
...
C***
	...
	complex*16 tmpx(12),tmpy(12),tmpz(12),tmpg(12)
	complex*16 dstrxx(12),dstrxy(12),dstrxz(12),dstryy(12),dstryz(12),dstrzz(12)
...
c--
c	Update the gravz array for azimuthal order m-minc and degrees l=m-minc through 1.6*mmax.
c	Compute the integrals over theta-r
c	Do spherical harmonic degrees up to min(2000,(8*mmax/5))
	  mval=m-minc
	  lmaxi=min(2000,(8*mmax/5))
c - - - - - -
	  do l=1,lmaxi
	  sumg1(l)=0.d0
	  sumg2(l)=0.d0
...
c	Use the z-point just above the current one.
	il2=N*(ilz)+ilx
	drho=(rho(ncx,ncz,il2)-rho(ncx,ncz,il))/((dz(ncz)/2.d0)*(x(ilz+1)-x(ilz)))
	endif
c	Factor of 1.e+4 converts the displacement coefficient to units of meters.
	...
	sumg1(l)=sumg1(l) + rlp2 * pb(indh,l+1) * bmpluz * drho * wt2(il) * dble(dx(ncx)/2.)
     &	* dble(dz(ncz)/2.) * stheta
	if(m.gt.minc) sumg2(l)=sumg2(l) + rlp2 * pb(indh,l+1) * bmminz * drho * wt2(il) * dble(dx(ncx)/2.)
     &	* dble(dz(ncz)/2.) * stheta
...
c	Next the exx contribution to dilatation
        dilap=dilap + bmplux*dphi2x(k,il)*dble(2./dx(ncx)) * fac1
        dilam=dilam + bmminx*dphi2x(k,il)*dble(2./dx(ncx)) * fac1
c	Next the ezz contribution to dilatation
	...
	dilam=dilam + bmminz*dphi2z(k,il)*dble(2./dz(ncz))
        ...
        enddo
cTE
	sumg1(l)=sumg1(l) + rlp2 * pb(indh,l+1) * dilap * rho(ncx,ncz,il) * wt2(il) * dble(dx(ncx)/2.)
     &	* dble(dz(ncz)/2.) * stheta
	if(m.gt.minc) sumg2(l)=sumg2(l) + rlp2 * pb(indh,l+1) * dilam * rho(ncx,ncz,il) * wt2(il)
...
c	Use the z-point in the cell just above the current one.
	...
	elseif(dabs(rho(ncx,ncz,il)-1.000d0).lt.1.d-6) then 
c	We're at the free surface -- implement a density jump unless water is in the shallowest layer.
	drho=0.d0
	else
	drho=-rho(ncx,ncz,il)
	endif
	sumg1(l)=sumg1(l) + rlp2 * pb(indh,l+1) * bmpluz * drho * wt(ilx) * dble(dx(ncx)/2.) * stheta
	if(m.gt.minc) sumg2(l)=sumg2(l) + rlp2 * pb(indh,l+1) * bmminz * drho * wt(ilx) * dble(dx(ncx)/2.) * stheta
	enddo
...
c	  pb(indh,l+1)=p(ind)*rtwopi
	    do iphi=1,41
	    phival=0.20*real(twopi)/(real(minc)*real(dsin(dlat)))*real(iphi-21)/20.
	    ang=dble(phival)*dble(m-minc)
	    facp=(dcos(ang)+ui*dsin(ang))*dble(minc)/twopi
	    facm=(dcos(ang)-ui*dsin(ang))*dble(minc)/twopi
	    gravz(indh,iphi,j)=gravz(indh,iphi,j) + facg * pb(indh,l+1) * (sumg1(l)*facp + sumg2(l)*facm)
	    enddo 
